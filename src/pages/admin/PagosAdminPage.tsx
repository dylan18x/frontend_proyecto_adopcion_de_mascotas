...
export default function PagosAdminPage() {
  ...
  const cargarDatos = async () => {
    try {
      ...
      // Adaptación para respuestas con paginación (items) o directas
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Payload limpio cumpliendo con el DTO (monto como número)
    const payload = {
  ...
  const prepararEdicion = (p: any) => {
    ...
    setFormData({
      ...
      // Extraemos el ID ya sea del objeto 'cliente' o de la propiedad plana
              ...
              <tr key={p.id_pago}>
                <td className="ps-4">
                  <div className="fw-bold text-dark">
                    {/* SOLUCIÓN AL "DESCONOCIDO": Si no hay objeto cliente, muestra un aviso */}
                    {p.cliente?.nombre || <span className="text-danger small">⚠️ Error: Cliente no vinculado</span>}
                  </div>
